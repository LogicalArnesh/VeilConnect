...
export default function ForgotUidPage() {
  ...
  const handleRequest = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!snap.empty) {
        ...
        await sendRecoveryEmail(email, code, 'uid');
        router.push(`/login/reset-auth?type=uid&email=${email}&uid=${user.id}`);
      } else {
    ...
    <AuthLayout>
      <div className="space-y-6">
        <div className="space-y-2 text-center">
          <h2 className="text-xl font-semibold tracking-tight">Recover User ID</h2>
          <p className="text-sm text-muted-foreground">Enter your email to receive a recovery key</p>
        </div>
