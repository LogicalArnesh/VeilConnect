...
const EMAIL_FOOTER = `
  <div style="margin-top: 40px; border-top: 2px solid ${COLOR_GREEN}; padding-top: 25px; color: #64748b; font-size: 11px; text-align: center; font-family: 'Inter', sans-serif;">
    <p style="margin: 0; font-weight: 800; color: ${COLOR_RED}; text-transform: uppercase; letter-spacing: 2px;">VeilConnect Operations Command</p>
    <p style="margin: 6px 0;">This is a secure, automated system message. Unauthorized access is strictly prohibited.</p>
    <p style="margin: 6px 0;">&copy; ${new Date().getFullYear()} Veil Confessions Intelligence Unit.</p>
    <p style="margin: 6px 0;">Official Support: <a href="mailto:veilconfessions@gmail.com" style="color: ${COLOR_GREEN}; font-weight: bold; text-decoration: none;">veilconfessions@gmail.com</a></p>
  </div>
`;
...
export async function sendConfessionAlertToAdmins(confessionData: any, adminEmails: string[]) {
  ...
  const mailPromises = recipients.map(email => 
    transporter.sendMail({
      ...
      to: email,
      subject: `[ALERT] New Mission Submission: #${confessionData.confessionNo}`,
      html: `
        <div style="background: #f1f5f9; padding: 40px; font-family: 'Inter', sans-serif;">
          <div style="max-width: 600px; margin: 0 auto; background: ${COLOR_WHITE}; border-radius: 24px; overflow: hidden; box-shadow: 0 20px 50px rgba(0,0,0,0.1); border: 1px solid #e2e8f0;">
            <div style="background: linear-gradient(135deg, ${COLOR_RED}, ${COLOR_GREEN}); padding: 50px 40px; text-align: center;">
              ${LOGO_HTML}
              <h1 style="color: ${COLOR_WHITE}; margin: 0; font-size: 28px; font-weight: 900; letter-spacing: -0.5px; text-transform: uppercase;">Incoming Submission</h1>
            </div>
            <div style="padding: 40px;">
              ...
              <div style="background: #f1f5f9; padding: 20px; border-radius: 12px; font-size: 13px; color: #64748b;">
                ...
                <p style="margin: 5px 0;"><strong>Log Index:</strong> <span style="color: ${COLOR_RED}; font-weight: bold;">#${confessionData.confessionNo}</span></p>
              ...
              <div style="margin-top: 35px; text-align: center;">
                <a href="https://veilconnect.netlify.app/dashboard" style="display: inline-block; padding: 16px 32px; background: ${COLOR_GREEN}; color: ${COLOR_WHITE}; text-decoration: none; border-radius: 12px; font-weight: 800; font-size: 14px; text-transform: uppercase; letter-spacing: 1px;">Open Command Center</a>
              </div>
            </div>
            ${EMAIL_FOOTER}
          </div>
        ...
        <div style="background: #f1f5f9; padding: 40px; font-family: 'Inter', sans-serif;">
          <div style="max-width: 600px; margin: 0 auto; background: ${COLOR_WHITE}; border-radius: 24px; overflow: hidden; border: 1px solid #e2e8f0;">
            ...
            <div style="padding: 40px; text-align: center;">
              <p style="font-size: 16px; color: #334155;">Greetings, <strong>${name}</strong>.</p>
              <p style="font-size: 14px; color: #64748b;">Use this authorization key to establish your operational identity:</p>
              ...
              <p style="font-size: 11px; color: #94a3b8;">Issued at: ${time}</p>
            </div>
            ${EMAIL_FOOTER}
          </div>
        </div>
    ...
    html: `
      <div style="background: #f1f5f9; padding: 40px; font-family: 'Inter', sans-serif;">
        <div style="max-width: 600px; margin: 0 auto; background: ${COLOR_WHITE}; border-radius: 24px; overflow: hidden;">
          ...
          <div style="padding: 40px; text-align: center;">
            <p style="font-size: 16px;">Greetings, ${name}. Your new role is:</p>
            ...
            <p style="font-size: 12px; color: #94a3b8;">Timestamp: ${time}</p>
          </div>
          ${EMAIL_FOOTER}
        </div>
      </div>
...
export async function notifyAdminsOfRequest(userData: any, adminEmails: string[]) {
  ...
  const mailPromises = recipients.map(email => 
    transporter.sendMail({
      from: '"VeilConnect Identity" <noreply.veilconfessions@gmail.com>',
      to: email,
      subject: `[ALERT] New Identity Request: ${userData.fullName}`,
      html: `
        <div style="background: #f1f5f9; padding: 40px; font-family: 'Inter', sans-serif;">
          <div style="max-width: 600px; margin: 0 auto; background: ${COLOR_WHITE}; border-radius: 24px; overflow: hidden; padding: 40px;">
            ${LOGO_HTML}
            <h2 style="color: ${COLOR_RED}; font-weight: 900; text-transform: uppercase;">New Access Request</h2>
            <div style="background: #f8fafc; padding: 25px; border-radius: 16px; border-left: 6px solid ${COLOR_GREEN}; margin: 25px 0;">
              ...
              <p style="margin: 8px 0; font-size: 14px;"><strong>UID:</strong> ${userData.id}</p>
            </div>
            <div style="text-align: center;">
              <a href="https://veilconnect.netlify.app/dashboard" style="display: inline-block; padding: 14px 28px; background: ${COLOR_RED}; color: ${COLOR_WHITE}; text-decoration: none; border-radius: 10px; font-weight: bold;">Review Operative</a>
            </div>
          </div>
          ${EMAIL_FOOTER}
        </div>
...
export async function sendResetConfirmationEmail(to: string, name: string, type: string) {
  ...
  await transporter.sendMail({
    ...
    to,
    subject: `Security Protocol: ${type.toUpperCase()} Updated`,
    html: `
      <div style="background: #f1f5f9; padding: 40px; font-family: 'Inter', sans-serif;">
        <div style="max-width: 600px; margin: 0 auto; background: ${COLOR_WHITE}; border-radius: 24px; padding: 40px; text-align: center;">
          ${LOGO_HTML}
          <h2 style="color: ${COLOR_GREEN}; font-weight: 900;">Security Update Successful</h2>
          <p>Your ${type === 'uid' ? 'User ID' : 'Passcode'} has been updated.</p>
